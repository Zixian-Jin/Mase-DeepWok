...
module sparse_simple_matmul_xcomp #(
    // Dimensions
    parameter N                    = 2,
    parameter M                    = 2,
    parameter K                    = 2,
    ...
    input  logic [X_WIDTH-1:0]   x_data [M_DENSE*N-1:0],
    // onehot-encoded control signals
    input  logic [BLOCK_NUM-1:0] x_nzc_flags,
    input  logic                 x_valid,
...
localparam ACC_WIDTH = X_WIDTH + Y_WIDTH + $clog2(BLOCK_SIZE*DENSE_BLOCK_NUM);
...
for (genvar i = 0; i < N; i++) begin : multi_row
        ...
        // Slice a single row of x. NOTE: `x_data` is already dense, so a router isn't needed
        logic [X_WIDTH-1:0] active_row_x [M_DENSE-1:0];
    ...
    for (genvar j = 0; j < K; j++) begin : multi_col
        ...
        logic [Y_WIDTH-1:0] active_col_y [M_DENSE-1:0];
