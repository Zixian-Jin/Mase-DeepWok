...
from mase_cocotb.matrix_tools import block_sparsify_matrix
...
class SimpleMatMulTB(Testbench):
    def __init__(self, dut) -> None:
        ...
        # sparsity-related settings
        _block_size = int(self.M / self.BLOCK_NUM)
        self.M_DENSE = _block_size * (self.BLOCK_NUM - self.SPARSE_BLOCK_NUM)
        ...
        # TODO: disable this line after debugging finishes
        dut.x_nzc_flags = 0b1101
    ...
    def generate_inputs(self, random=False):
        ...
        # sparsification
        X = block_sparsify_matrix(X, self.BLOCK_NUM, self.SPARSE_BLOCK_NUM)
        
        X_input = X.flatten().tolist()
        X_input.reverse()
        Y_input = Y.flatten().tolist()
        Y_input.reverse()
        return X_input, X_input[:self.N * self.M_DENSE], Y_input
...
async def small_positive_nums(dut):
    ...
    X, X_dense, Y = tb.generate_inputs()
    exp_out = tb.model(X, Y)
    tb.x_driver.append(X_dense)
